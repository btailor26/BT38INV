...
def push_group_stock(group_id):
    """Compatibility shortcut: group push now routes through WarehousePushCoordinator."""
    try:
        group = db.session.get(ProductGroup, group_id)
        if not group:
        ...
        for item in group.items:
            if item.sku and item.sku not in skus_to_push:
                skus_to_push.append(item.sku)

        if not skus_to_push:
            return jsonify({'success': False, 'message': 'No SKUs found for group'}), 400

        from warehouse_push_coordinator import WarehousePushCoordinator
        coordinator = WarehousePushCoordinator()
        prepared_count = coordinator.prepare_for_items(skus_to_push, operation='group_push')
        jobs_enqueued = coordinator.enqueue_pending_jobs()

        return jsonify({
            'success': True,
            'message': f'Group push queued through unified warehouse path for {len(skus_to_push)} SKU(s)',
            'group_id': group_id,
            'skus': skus_to_push,
        ...
        db.session.rollback()
        logging.error(f'Error queueing group push: {str(e)}', exc_info=True)
...
def _governed_push_shortcut(store, item):
    """Single shortcut gate for legacy push buttons; queues through WarehousePushCoordinator."""
    settings = PushSettings.get_or_create_settings()
    if not settings.global_push_enabled:
        return False, "Push blocked: global Fuse Box push is disabled"
    if not store.is_active:
        return False, f"Push blocked: {store.name} is inactive"
    if hasattr(store, "auto_push_enabled") and store.auto_push_enabled is False:
        return False, f"Push blocked: {store.name} push is disabled in Fuse Box"
    if not store.api_key:
        return False, "No API credentials configured"
    if not item or not getattr(item, "sku", None):
        return False, "Push blocked: item SKU missing"

    from warehouse_push_coordinator import WarehousePushCoordinator
    coordinator = WarehousePushCoordinator()
    prepared_count = coordinator.prepare_for_items([item.sku], operation="shortcut_push")
    jobs_enqueued = coordinator.enqueue_pending_jobs()

    if jobs_enqueued > 0:
        return True, f"Queued {jobs_enqueued} push job(s) through unified warehouse path"
    if prepared_count > 0:
        return True, f"Prepared {prepared_count} push action(s) through unified warehouse path"
    return False, "No eligible marketplace push jobs were queued"
            ...
            for item in batch_items:
                ...
                for store in item_stores:
                    try:
                        ...
                        success, message = _governed_push_shortcut(store, item)
...
def update_store_push_settings(store_id):
    """Update push settings for a specific store"""
    try:
        store = db.session.get(Store, store_id)
        if not store:
        ...
        # Update store-specific push settings
        if 'auto_push_enabled' in data:
            store.auto_push_enabled = data['auto_push_enabled']
        if 'push_priority' in data:
            store.push_priority = data['push_priority']
        if 'push_frequency_minutes' in data:
            store.push_frequency_minutes = data['push_frequency_minutes']
        if 'push_batch_size' in data:
            store.push_batch_size = data['push_batch_size']
        if 'push_on_quantity_change' in data:
            store.push_on_quantity_change = data['push_on_quantity_change']
        if 'push_on_price_change' in data:
            store.push_on_price_change = data['push_on_price_change']
        if 'push_on_item_create' in data:
            store.push_on_item_create = data['push_on_item_create']
        if 'push_on_item_update' in data:
            store.push_on_item_update = data['push_on_item_update']
        if 'max_retry_attempts' in data:
            store.max_retry_attempts = data['max_retry_attempts']
        if 'auto_disable_on_failures' in data:
            store.auto_disable_on_failures = data['auto_disable_on_failures']
        if 'failure_threshold' in data:
            store.failure_threshold = data['failure_threshold']
        if 'immediate_push' in data:
            store.immediate_push = data['immediate_push']
        if 'large_change_confirmation' in data:
            store.large_change_confirmation = data['large_change_confirmation']
        if 'large_change_threshold' in data:
            store.large_change_threshold = data['large_change_threshold']
        
        # Reset failure count if auto_disable_on_failures is turned off
        if 'auto_disable_on_failures' in data and not data['auto_disable_on_failures']:
            store.current_failure_count = 0
        ...
        return jsonify({
            'success': True,
            'message': f'Push settings updated for {store.name}',
            'store_settings': store.to_dict()
        })
        
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error updating store push settings: {str(e)}")
        return jsonify({'success': False, 'error': str(e)})
